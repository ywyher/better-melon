 ...
 * @param params The parameters for decompression
 * @param params.buffer Base64 encoded buffer to decompress
 * @param params.contentEncoding Content-Encoding header value
 * @returns Base64 encoded decompressed buffer
 */
export default async function decompress(params: { 
  buffer: string; 
  contentEncoding?: string; 
}): Promise<string> {
  // Convert base64 string back to buffer
  const inputBuffer = Buffer.from(params.buffer, 'base64');
  ...
  try {
    ...
    if (params.contentEncoding) {
      const encoding = params.contentEncoding.toLowerCase().trim();
